...
작업대의 `3단계 준비` 탭에서 로컬 표시 이름 역할, 결정 메모, 감사 이력, 197개 가이드 기반 보조 제안(미리보기→사람 승인)을 사용할 수 있습니다. 계정 동기화·초대 메일·결제·외부 AI는 기본 차단되어 있으며, 제안을 승인해도 진단 점수와 증거는 바뀌지 않습니다. 실제 상용 기능으로 전환하려면 `detailed-development-plan-vNEXT.md`의 S3-01~04 운영 게이트를 먼저 충족해야 합니다.
전체 작업대 백업에는 Stage 3 기록도 포함되며, 화면에서 Stage 3 전용 JSON 저장·복원을 별도로 사용할 수 있습니다.
